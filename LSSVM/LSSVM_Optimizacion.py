...
parameters = {'C': [0.1, 1, 10, 100], 'gamma': [0.001, 0.01, 0.1, 1, 10], 'epsilon': [0.001, 0.01, 0.1, 1]}

# Definición de la SVM con kernel RBF
svm_model = SVR(kernel='rbf')

# Búsqueda exhaustiva de los mejores parámetros con validación cruzada
...
best_svm = SVR(kernel='rbf', C=clf.best_params_['C'], gamma=clf.best_params_['gamma'], epsilon=clf.best_params_['epsilon'])
...
plt.scatter(y_test, y_pred, color='blue', label="Datos Reales vs. Predichos")
...
plt.title("Comparación de Valores Reales y Predichos por SVR")
plt.legend()
plt.grid(True)
plt.show()
